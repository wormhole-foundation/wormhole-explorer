...
import (
	...
	sqsclient "github.com/wormhole-foundation/wormhole-explorer/common/client/sqs"
...
func NewEventSqs(consumer *sqsclient.Consumer, converter ConverterFunc, metrics metrics.Metrics, logger *zap.Logger, opts ...SQSOption) *SQS {
	s := &SQS{
		consumer:  consumer,
		chSize:    10,
		metrics:   metrics,
...
// WithChannelSize allows to specify a channel size when setting a value.
		...
		for {
			...
			for _, msg := range messages {
				...
				q.ch <- &sqsConsumerMessage{
					...
					consumer:      q.consumer,
					expiredAt:     expiredAt,
					sentTimestamp: sqsclient.GetSentTimestamp(msg),
					retry:         uint8(retry),
					metrics:       q.metrics,
					ctx:           ctx,
				}
