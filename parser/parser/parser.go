...
func (c ParserVAAAPIClient) Parse(chainID uint16, address string, sequence uint64, vaa []byte) (*ParseVaaResponse, error) {
	endpointUrl := fmt.Sprintf("%s/vaa-payload-parser/parser/vaa/%v/%s/%v", c.BaseURL, chainID,
		address, sequence)
	...
	if err != nil {
		c.Logger.Error("error marshalling payload", zap.Error(err), zap.Uint16("chainID", chainID),
			zap.String("address", address), zap.Uint64("sequence", sequence))
		return nil, err
	}
