...
type LegacyGuardianSetHistory struct {
...
func (h *LegacyGuardianSetHistory) Verify(ctx context.Context, vaa *sdk.VAA) error {
...
func (h LegacyGuardianSetHistory) GetLatest() common.GuardianSet {
	return h.guardianSetsByIndex[len(h.guardianSetsByIndex)-1]
}

// Get get guardianset config by enviroment.
func GetByEnv(enviroment string, alertClient alert.AlertClient) LegacyGuardianSetHistory {
	switch enviroment {
	case domain.P2pTestNet:
		return getTestnetGuardianSetLegacy(alertClient)
	default:
		return getMainnetGuardianSetLegacy(alertClient)
	}
}

func getTestnetGuardianSetLegacy(alertClient alert.AlertClient) LegacyGuardianSetHistory {
	...
	return LegacyGuardianSetHistory{
...
func getMainnetGuardianSetLegacy(alertClient alert.AlertClient) LegacyGuardianSetHistory {
	...
	gs4ValidUntil := time.Now().Add(tenYears) // still valid so we add 10 years
	gs4 := common.GuardianSet{
		Index: 4,
		Keys: []eth_common.Address{
			eth_common.HexToAddress("0x5893B5A76c3f739645648885bDCcC06cd70a3Cd3"), // RockawayX
			eth_common.HexToAddress("0xfF6CB952589BDE862c25Ef4392132fb9D4A42157"), // Staked
			eth_common.HexToAddress("0x114De8460193bdf3A2fCf81f86a09765F4762fD1"), // Figment
			eth_common.HexToAddress("0x107A0086b32d7A0977926A205131d8731D39cbEB"), // ChainodeTech
			eth_common.HexToAddress("0x8C82B2fd82FaeD2711d59AF0F2499D16e726f6b2"), // Inotel
	...
	return LegacyGuardianSetHistory{
		guardianSetsByIndex:    []common.GuardianSet{gs0, gs1, gs2, gs3, gs4},
		expirationTimesByIndex: []time.Time{gs0ValidUntil, gs1ValidUntil, gs2ValidUntil, gs3ValidUntil, gs4ValidUntil},
		alertClient:            alertClient,
	}
}
