...
func Test_buildAppActivityQuery(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			withoutTabs := strings.ReplaceAll(query, "\t", "")
			assert.Equal(t, tc.expectedAppQuery, withoutTabs)
			totalsQuery := repository.buildTotalsAppActivityQuery(tc.input)
			withoutTabs = strings.ReplaceAll(totalsQuery, "\t", "")
			assert.Equal(t, tc.expectedTotalsQuery, withoutTabs)
		})
	}
