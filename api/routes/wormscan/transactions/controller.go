...
func (c *Controller) ListTransactions(ctx *fiber.Ctx) error {
	...
	for i := range queryResult.Transactions {
		...
		response.Transactions[i].OriginChain = queryResult.Transactions[i].EmitterChain

		// Some VAAs are not generated by the token bridge
		if len(queryResult.Transactions[i].ParsedVaa) == 1 {
			response.Transactions[i].DestinationAddress = queryResult.Transactions[i].ParsedVaa[0].Result.ToAddress
			response.Transactions[i].DestinationChain = queryResult.Transactions[i].ParsedVaa[0].Result.ToChain
		}
