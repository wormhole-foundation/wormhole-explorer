...
func (c *Controller) ListTransactions(ctx *fiber.Ctx) error {
	...
	for i := range queryResult.Transactions {
		...
		if len(queryResult.Transactions[i].Vaas) == 1 {
			tx.Timestamp = queryResult.Transactions[i].Vaas[0].Timestamp
		}
		...
		} else if len(queryResult.Transactions[i].VaaIdTxHash) == 1 {
			tx.TxHash = queryResult.Transactions[i].VaaIdTxHash[0].TxHash
		}
